...
const ContactSection = ({ sectionIcon, title, leadParagraph, contactInfoSource, customPhone, customEmail, button, backgroundStyle, entryId }) => {
    const buttonId = button?.sys?.id
    button = button?.fields
    const bgClass = backgroundStyle === "Beige Background" ? "bg-beige" : "bg-default";
    const phone = contactInfoSource === 'Custom' ? customPhone : 'NO DEFAULT PHONE';
    const email = contactInfoSource === 'Custom' ? customEmail : 'NO DEFAUTL EMAIL';
        ...
        <section className={`contact-section ${bgClass}`} id={sectionId}>
            <div className="container">
                ...
                <div className="contact-info">
                    ...
                    {email && <p {...inspectorProps({ fieldId: 'customEmail' })}>Email: {email}</p>}
                </div>
